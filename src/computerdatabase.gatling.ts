...
import BackendService, { Method } from "./actions/backend/backend-actions";
...
export default simulation((setUp) => {
  ...
  const search = exec(
    ...
    pause(1),
    feed(feeder),
    backendService.searchEndpoint
      .buildHttp()
      .formParam("f", "#{searchCriterion}")
      .check(css("a:contains('#{searchComputerName}')", "href").saveAs("computerUrl")),
    pause(1),
    selectEndpoint.buildHttp().check(status().is(200)),
    pause(1)
  );
